...
if __name__ == "__main__":
    ...
    INFO_TEXT = "[INFO] #splite train: {}, #split test: {}".format(
        n_split_train, n_split_test)
    send_msg_to_dingtalk(info_text=INFO_TEXT, is_send_msg=SENDING_TRAINING_INFO)
    ...
    oof_pred_label = pd.DataFrame(None, index=train_group_id)
    oof_pred_label["labels"] = split_labels
    oof_pred_label = oof_pred_label.groupby(oof_pred_label.index).values
